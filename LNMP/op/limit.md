...
[参考](http://www.cnblogs.com/clds/p/5850066.html)
...
那么是怎么解决计数器算法的bug呢？
    * 我们可以看到59s发送100个请求，最后一个小窗口计数器为100（灰色小格子），整个大窗口此时也是100个。那么滑动到下一秒中（最后一个小窗口为倒数第二个还是灰色那个，新1s为最后一个小窗口红色那个），如果发现又来了100个请求，那么最后一个小窗口计数器逐步增加100，但是加到1的时候就发现整个大窗口此时已经达到了101触发了限流。
...
从上可以看到令牌桶算法比漏斗算法要复杂：我们有一个**恒定大小且以恒定速率加令牌的桶**。如果令牌数超过桶大小就抛弃，每个请求过来可以拿走桶里的令牌，如果没有得到足够令牌就被限流抛弃。
####代码实现
```php
...
###总结
* 计数器 VS 滑动窗口
    * 计数器算法是最简单的算法，可以看成是滑动窗口的低精度实现。滑动窗口由于需要存储 多份的计数器（每一个格子存一份），所以滑动窗口在实现上需要更多的存储空间。也就是说，如果滑动窗口的精度越高，需要的存储空间就越大。
* 漏桶算法 VS 令牌桶算法
